...
public class InterpValueHistoryBuffer {
     ...
     */
	public InterpValueHistoryBuffer(int length, double init_val){
	 ...
	 * @param time Time at which the value was sampled. elements must be inserted in a monotomically increasing fashion
	...
	public double getValAtTime(double time){
		...
		while(true){
			if((time_buffer[lower_idx] <= time) & (time_buffer[upper_idx] >= time)){
				//Case, we've found the desired block. Calculate the value at the given time with linear interpolation
